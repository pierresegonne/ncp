...
def visualize_model(
    filename, sess, graph, has_uncertainty, dataset, visibles, batch_size=100
):
    ...
    for index, ax in enumerate(axes):
        ...
        ax.scatter(
            ...
            dataset.train.targets[visible, 0],
            c="#000000",
            lw=0,
            s=4,
        )
...
def plot_prediction(ax, inputs, mean, std, area=2):
    ...
    ax.fill_between(inputs, mean - area * std, mean + area * std, alpha=0.1, lw=0)
